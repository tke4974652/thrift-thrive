...
const handleUpload = async () => {
  ...
  if (currentUser && selectedFile.value) {
    ...
    const sellData = {
      ...
      userName: userName.value, // Get the user's display name
    ...
    isLoading.value = false; // Stop loading overlay
  } else {
    alert('User is not logged in or no file selected.');
    isLoading.value = false; // Stop loading overlay in case of error
  }
};
